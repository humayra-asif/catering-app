  ...
  Future<String> getCatererName(String catererId) async {
    try {
      final snapshot = await FirebaseFirestore.instance
          .collection('companyInfo') // 👈 Adjust this if your caterer collection has a different name
          .doc(catererId)
          .get();
      if (snapshot.exists) {
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final selectedDate = (order['selectedDate'] as Timestamp?)?.toDate();
                    ...
                    return FutureBuilder<String>(
                      ...
                      builder: (context, catererSnapshot) {
                        final catererName = catererSnapshot.data ?? 'Loading...';

                        return Card(
                          color: isCancelled ? AppColors2.grey : Colors.white,
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text("Dish: $foodName", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                SizedBox(height: 4),
                                Text("Caterer: $catererName", style: TextStyle(color: Colors.black87)),
                                if (selectedDate != null)
                                  Text("Selected Date: ${DateFormat('dd/MM/yyyy').format(selectedDate)}"),
                                if (createdAt != null)
                                  Text("Order Date: ${DateFormat('dd/MM/yyyy hh:mm a').format(createdAt)}"),
                                Text("Total: $total", style: TextStyle(fontWeight: FontWeight.bold)),
                                SizedBox(height: 10),
                                if (isCancelled)
                                  Text("Order Cancelled", style: TextStyle(color: AppColors.red, fontWeight: FontWeight.bold))
                                else
                                  Align(
